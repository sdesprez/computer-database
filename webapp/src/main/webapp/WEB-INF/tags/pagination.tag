...
<ul class="pagination">
	<c:if test="${pageNumber > 0}">
		<li><h:link target="${target}" size="${size}" search="${search}" sort="${sort}" direction="${direction}"><spring:message code="button.first"/></h:link></li>
	...
	<c:if test="${pageNumber-2 > 0}">
		<li><h:link target="${target}" page="${pageNumber-3}" size="${size}" search="${search}" sort="${sort}" direction="${direction}">${pageNumber-2}</h:link></li>
	</c:if>
	...
	<c:if test="${pageNumber < pages-1}">
		<li><h:link target="${target}" page="${pageNumber+1}" size="${size}" search="${search}" sort="${sort}" direction="${direction}">&raquo;</h:link></li>
		<li><h:link target="${target}" page="${pages-1}" size="${size}" search="${search}" sort="${sort}" direction="${direction}"><spring:message code="button.last"/></h:link></li>
	</c:if>
</ul>
